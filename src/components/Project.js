...
class Project extends Component {
  ...
  getQuote() {
    ...
    this.setState({
      qtTitle: quoteJSON["quotes"][randQ]["qt-title"],
      qtDesc: quoteJSON["quotes"][randQ]["qt-desc"],
    });
  }
  ...
  getProject() {
    ...
    console.log(maxm, minm, randQ);
    this.setState({
      projTitle: projJSON["projects"][randQ]["proj-title"],
      projImg: projJSON["projects"][randQ]["proj-img"],
      projDesc: projJSON["projects"][randQ]["proj-desc"],
      projTags: projJSON["projects"][randQ]["proj-tags"],
    });
  }
          ...
          <div className="column is-4 is-offset-1">
            ...
            <div className="box">
              <figure className="image container is-128x128 wow animate__animated animate__bounce">
                <img
                  className="is-rounded proj-logo"
                  alt="proj-logo"
                  src={this.state.projImg}
                />
              </figure>
              <div className="proj-title has-text-centered">
                {this.state.projTitle}
              </div>
              <div className="proj-desc">{this.state.projDesc}</div>
              <div className="tags are-medium is-right proj-tags">
                {this.state.projTags.map((elem) => (
                  <span className="tag is-black">{elem}</span>
                ))}
              </div>
              <button
                className="button is-rounded randombtn is-centered"
          ...
          <div className="column is-3">
            ...
            <div className="box">
              <div className="myquote">
                ...
                <div className="qt-desc">{this.state.qtDesc}</div>
              </div>
              <button
                className="button is-rounded randombtn"
